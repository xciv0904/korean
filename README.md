...
- **技術選擇**:這個網站是純前端靜態網站(部署在 GitHub Pages,沒有自己的後端伺服器)。像 Google Cloud Translation、Naver Papago 這類品質較好的翻譯 API 都需要 API 金鑰,金鑰寫在前端程式碼裡任何人打開瀏覽器開發者工具都看得到,會被盜用來刷你的額度,所以沒辦法直接用。改用 [MyMemory Translation API](https://mymemory.translated.net/):不需要金鑰、支援瀏覽器直接呼叫(有開放 CORS)、免費額度個人使用足夠。
- **注意事項**:機器翻譯,語氣、口語程度不一定準確,畫面上已經加了提醒(僅供參考,正式場合建議再跟母語人士確認)。這個功能需要連網才能用,跟原本錄音/SRS 複習(完全存在本機,離線也能用)是分開的,沒有連網時只有這個分頁會顯示錯誤訊息,其他功能不受影響。單次查詢字數上限 400 字(超過會提示分段翻譯)。
- 程式碼在 `src/hooks/useTranslate.ts`(呼叫 API、處理 loading/錯誤狀態)+ `src/components/TranslateTool.tsx`(輸入框 + 翻譯按鈕 + 結果顯示)。
